...
const classValidationPatterns = {
  ...
  PROPERTY_MUST_BE_NOSQL_ID: '{property} must be a valid NoSQL ID',
  PROPERTY_MUST_BE_SQL_ID: '{property} must be a valid SQL ID',
  ...
  private async getMessage(
    ...
    if (!error.children || !error.children.length) {
      ...
      return patternFinder
        ? this.i18n.translate(`validation.${patternFinder}`, {
